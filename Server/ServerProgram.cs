...
namespace Server
{
    internal class ServerProgram
    {
        ...
        public static async Task ClearDatabaseTables(DatabaseFacade database)
        {
            ...
            {
                var checkTableExistsSql = $"SELECT CASE WHEN EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{table}' AND TABLE_SCHEMA = 'dbo' AND TABLE_CATALOG = 'NServiceBusDb') THEN 1 ELSE 0 END";

                using (var command = database.GetDbConnection().CreateCommand())
                {
                    command.CommandText = checkTableExistsSql;
                    database.GetDbConnection().Open(); // Make sure the connection is open
                    ...
                    database.GetDbConnection().Close(); // Close the connection after execution
                }
            }
        }
